...
export async function POST(request: Request) {
  try {
    ...
    // Envia para C2S
    const c2sRes = await fetch(
      `${process.env.C2S_BASE_URL}/leads`,
      {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json(
      { success: true, lead_id: data.id || 'ok' },
      { status: 200 }
    );
    ...
    return NextResponse.json(
      { success: true, warning: 'Lead salvo em log.' },
      { status: 200 }
    );
  }
}
